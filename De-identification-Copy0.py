...
engine = create_engine('sqlite:///kaPC_1-17-4-17-14-3.db')

# <codecell>

cnx = engine.raw_connection()
xx = pd.io.sql.read_frame('SELECT * FROM source',cnx)

# <codecell>

for item in xx.columns[:]:
    del xx[item]
xx.columns

# <codecell>

xx_exp_cert = xx['certified']==1
...
selUnique(c,"original","final_cc_cname")
...
qry = selUnique(c,"original","final_cc_cname")
len(qry)
...
countryNamer(c,"original",countryVar)
...
csvExport(c, table,"HMXPC13_DI_4-18-14.csv")

# <codecell>

c.execute("SELECT SUM(Count) FROM source WHERE last_event = ''")
...
csvExport(c, table,"HMXPC13_DI_v2_5-14-14.csv")
...
c.execute("SELECT SUM(Count), kkey, course_id FROM source GROUP BY course_id")
qry = c.fetchall()
qry
